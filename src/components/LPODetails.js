...
const LPODetails = ({ lpo, onUpdate, onClose, isViewOnly = false }) => {
  const [formData, setFormData] = useState(lpo);
    ...
    >
      ...
      >
        ...
        <h2 className="text-base sm:text-lg font-bold mb-2 sm:mb-3 text-center">
          {isViewOnly ? "LPO Details" : "Edit LPO"}
        </h2>

        <div className="grid grid-cols-2 gap-2 text-xs sm:text-sm">
          <div>
            <label className="block font-medium">LPO #</label>
            <input
              type="text"
              name="lpoNumber"
        ...
        <div className="border rounded p-2 mb-3 text-xs">
          <div className="grid grid-cols-4 gap-1 font-bold border-b pb-1 mb-1">
          ...
          {formData.items.map((item, index) => (
            <div key={index} className="grid grid-cols-4 gap-1 border-b py-1">
              <input
                type="text"
